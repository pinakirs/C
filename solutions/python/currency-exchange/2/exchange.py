...
def get_leftover_of_bills(amount, denomination):
    ...
    no_of_bills = get_number_of_bills(amount, denomination)
    return (amount - (denomination * no_of_bills))
...
def exchangeable_value(budget, exchange_rate, spread, denomination):
    ...
    """
    exchange_rate_with_spread = round((exchange_rate + ((spread * exchange_rate)/ 100)), 2)
    exchanged_value = exchange_money(budget, exchange_rate_with_spread)
    no_of_bills = get_number_of_bills(exchanged_value, denomination)
    return int(no_of_bills * denomination)
