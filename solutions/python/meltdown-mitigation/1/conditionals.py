...
def reactor_efficiency(voltage, current, theoretical_max_power):
    ...
    if efficiency >= 80:
        efficiency_code = "green"
    elif efficiency < 80 and efficiency >= 60:
        efficiency_code = "orange"
    elif efficiency < 60 and efficiency >= 30:
        efficiency_code = "red"
    else:
        efficiency_code = "black"
    
    return efficiency_code
...
def fail_safe(temperature, neutrons_produced_per_second, threshold):
    ...
    low_threshold_boundary = threshold * 90/100
    normal_lower_boundary = threshold  - (threshold *  10/100)
    normal_upper_boundary = threshold  + (threshold *  10/100)
    indicator = temperature * neutrons_produced_per_second

    if indicator < low_threshold_boundary:
        status = "LOW"
    elif indicator > normal_lower_boundary and indicator < normal_upper_boundary:
        status = "NORMAL"
    else:
        status = "DANGER"
